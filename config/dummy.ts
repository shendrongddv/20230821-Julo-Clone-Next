// BrandLogo
export const DataBrandLogo = [
  {
    id: 2,
    name: "NPM",
    media: "npm.svg",
  },
  {
    id: 3,
    name: "MongoDB",
    media: "mongodb.svg",
  },
  {
    id: 7,
    name: "WordPress",
    media: "wordpress.svg",
  },
  {
    id: 5,
    name: "GO",
    media: "go.svg",
  },
  {
    id: 6,
    name: "Ruby On Rails",
    media: "ruby-on-rails.svg",
  },
  {
    id: 9,
    name: "jQuery",
    media: "jquery.svg",
  },
  {
    id: 8,
    name: "Flask",
    media: "flask.svg",
  },
  {
    id: 4,
    name: "Kotlin",
    media: "kotlin.svg",
  },
  {
    id: 10,
    name: "django",
    media: "django.svg",
  },
  {
    id: 11,
    name: "Swift",
    media: "swift.svg",
  },
  {
    id: 12,
    name: "Symfony",
    media: "symfony.svg",
  },
  {
    id: 1,
    name: "Python",
    media: "python.svg",
  },
];

// Banners Section
export const contentBanners = [
  // {
  //   id: 1,
  //   label: "Banner 1",
  //   media: { desktop: "banner-1.webp", mobile: "1-mobile.png" },
  //   url: "/",
  // },
  // {
  //   id: 2,
  //   label: "Banner 2",
  //   media: { desktop: "banner-2.webp", mobile: "2-mobile.png" },
  //   url: "/",
  // },
  {
    id: 3,
    label: "Banner 3",
    media: { desktop: "banner-3.webp", mobile: "3-mobile.webp" },
    url: "/",
  },
  {
    id: 4,
    label: "Banner 4",
    media: { desktop: "banner-4.webp", mobile: "4-mobile.webp" },
    url: "/",
  },
  {
    id: 5,
    label: "Banner 5",
    media: { desktop: "banner-5.webp", mobile: "5-mobile.webp" },
    url: "/",
  },
  // {
  //   id: 6,
  //   label: "Banner 6",
  //   media: { desktop: "banner-6.webp", mobile: "6-mobile.png" },
  //   url: "/",
  // },
];

// Promotions
export const contentPromotions = [
  {
    id: 1,
    label: "Promo 1",
    media: { desktop: "promotion-1.webp", mobile: null },
    url: "/",
  },
  {
    id: 2,
    label: "Promo 2",
    media: { desktop: "promotion-2.webp", mobile: null },
    url: "/",
  },
  {
    id: 3,
    label: "Promo 3",
    media: { desktop: "promotion-3.webp", mobile: null },
    url: "/",
  },
  {
    id: 4,
    label: "Promo 4",
    media: { desktop: "promotion-4.webp", mobile: null },
    url: "/",
  },
];

//
export const contentFastAndSecureProcess = [
  {
    id: 1,
    title: "Pinjaman Online Bunga Rendah",
    body: "JULO menggunakan teknologi big data, machine learning & algoritma mutakhir untuk menguji kelayakan kredit kamu.",
    media: "Teknologi Canggih.svg",
  },
  {
    id: 2,
    title: "Pinjaman Online Terpercaya",
    body: "JULO paham bahwa data nasabah merupakan hal yang sangat berharga. Oleh karena itu, JULO pastikan bahwa data kamu tersimpan dengan aman.",
    media: "Sistem Keamanan.svg",
  },
  {
    id: 3,
    title: "Pinjaman Online Legal",
    body: "Mengikuti standar tingkat bunga dari AFPI, JULO memberlakukan tingkat bunga rendah serta transparan dari 0,1% per hari.",
    media: "Bunga Rendah.svg",
  },
];

//
export const contentYourNeed = [
  {
    id: 1,
    title: "Pinjaman Tunai",
    media: "1.svg",
  },
  {
    id: 2,
    title: "Transfer Tunai",
    media: "2.svg",
  },
  {
    id: 3,
    title: "Top Up E-Wallet",
    media: "3.svg",
  },
  {
    id: 4,
    title: "Pulsa & Data",
    media: "4.svg",
  },
  {
    id: 5,
    title: "Bayar Listrik",
    media: "5.svg",
  },
  {
    id: 6,
    title: "E-commerce",
    media: "6.svg",
  },
  {
    id: 7,
    title: "BPJS Kesehatan",
    media: "7.svg",
  },
  {
    id: 8,
    title: "Biaya Sekolah",
    media: "8.svg",
  },
  {
    id: 9,
    title: "Tiket Kereta",
    media: "9.svg",
  },
];

// Reviews
export const contentReviews = [
  {
    id: 1,
    name: "Andri Yanto",
    age: 29,
    job: "Karyawan Swasta",
    review:
      "Awal kenal JULO dari media sosial dan JULO sangat membantu untuk renovasi rumah dan memenuhi kebutuhan saya sehari-hari. Ditambah lagi, sering juga dapat cashback setiap saya bayar cicilan sebelum jatuh tempo. Rasanya makin senang dan betah pakai JULO!",
    avatar: "user-1.png",
  },
  {
    id: 2,
    name: "Kristina",
    age: 35,
    job: "Ibu Rumah Tangga",
    review:
      "Waktu itu lagi coba-coba cari aplikasi kredit terpercaya, liat review JULO bagus dan ternyata memang sangat membantu. Segala kebutuhan dan keperluan bayar kuliah adik bisa langsung terpenuhi pakai limit kredit digital JULO.",
    avatar: "user-2.png",
  },
  {
    id: 3,
    name: "Nendi",
    age: 32,
    job: "Enterpreneur",
    review:
      "Lihat JULO dari iklan, saya coba cari tahu lebih jauh dan ternyata proses pengajuannya sangat mudah dan praktis. Sudah coba 3 kali tarik dana untuk modal usaha dan semuanya memuaskan. Terima kasih JULO sudah membantu saya!",
    avatar: "user-3.png",
  },
];

// Partners
export const contentPartners = [
  {
    id: 1,
    label: "Tokopedia",
    media: "Tokopedia.png",
  },
  {
    id: 2,
    label: "Grab",
    media: "Grab.png",
  },
  {
    id: 3,
    label: "Bank Sampoerna",
    media: "Bank Sampoerna.png",
  },
  {
    id: 4,
    label: "Sepulsa",
    media: "Sepulsa.png",
  },
  {
    id: 5,
    label: "DOKU Wallet",
    media: "DOKU Wallet.png",
  },
];

// Media
export const contentMedia = [
  {
    id: 1,
    label: "Liputan 6",
    media: "Liputan 6.png",
  },
  {
    id: 2,
    label: "Market Insider",
    media: "Market Insider.png",
  },
  {
    id: 3,
    label: "Techcrunch",
    media: "Techcrunch.png",
  },
  {
    id: 4,
    label: "Kontan",
    media: "Kontan.png",
  },
  {
    id: 5,
    label: "Media Indonesia",
    media: "Media Indonesia.png",
  },
  {
    id: 6,
    label: "Daily Social",
    media: "Daily Social.png",
  },
  {
    id: 7,
    label: "SWA",
    media: "SWA.png",
  },
  {
    id: 8,
    label: "Techinasia",
    media: "Techinasia.png",
  },
  {
    id: 9,
    label: "VIVA",
    media: "VIVA.png",
  },
  {
    id: 10,
    label: "Forbes",
    media: "Forbes.png",
  },
  {
    id: 11,
    label: "Sindonews",
    media: "Sindonews.png",
  },
  {
    id: 12,
    label: "Medcom",
    media: "Medcom.png",
  },
  {
    id: 13,
    label: "Katadata",
    media: "Katadata.png",
  },
  {
    id: 14,
    label: "Kompas",
    media: "Kompas.png",
  },
];
